### 简介
本项目是由 YunLe 提供的用于上位机控制云乐线控底盘的驱动程序，用户可使用该驱动程序配合 CAN 连接设备在 ROS2 环境下完成对底盘的控制。
...
1. 创建 ROS2 工作空间，并在其中创建 `src` 目录。
2. 将 `chassis_driver` 和 `yunle_msgs` 两个包放入 `src` 目录。
3. 在工作空间根目录执行：`colcon build --symlink-install`。
4. 执行：`source ./install/setup.bash`。
5. （可选）若运行时提示找不到 `libcontrolcan.so`，可执行：

   `sudo cp ./src/chassis_driver/chassis_driver/lib/x86/linux_64/libcontrolcan.so /usr/lib`

6. 启动驱动：`ros2 launch chassis_driver chassis_driver.launch.py`。

**注意**：使用以太网 CAN 网关前，请先完成 CAN 盒网络参数配置（参考设备文档）。

> 建议：将 `car_type`、`local_ip`、`local_port` 与实际车辆/网段保持一致，错误参数会导致无法收发 CAN 数据。

### launch文件配置
---

.launch 文件位于 `./chassis_driver/chassis_driver/launch/chassis_driver.launch.py`。根据所要控制的车型和上位机 IP，调整参数配置。示例如下：
...
|       名称       |                可选值                 |            释义            |
| :--------------: | :-----------------------------------: | :------------------------: |
|    canbus_type   |                0 或 1                 | 默认为 1，表示以太网 UDP 模式 |
...
具体的消息数据格式可参考 `./src/yunle_msgs/msg` 下的消息定义。
